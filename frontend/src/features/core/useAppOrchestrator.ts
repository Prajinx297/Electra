...
export const useAppOrchestrator = () => {
  ...
  const sessionPayload = useMemo<SessionPayload>(() => ({
    journeyId, currentState, stateHistory: history, oracleHistory,
    cognitiveLevel, language, bookmarkedStates, completedJourneys, profile
  }), [journeyId, currentState, history, oracleHistory, cognitiveLevel, language, bookmarkedStates, completedJourneys, profile]);
  ...
  const handleOnboardingComplete = useCallback(async (nextProfile: OnboardingProfile) => {
    ...
    civicBus.emit({ type: "SCORE_EARNED", payload: { points: 50, reason: "Complete onboarding" } });
  ...
  const handleStreamComplete = useCallback(async (response: OracleResponse) => {
    ...
    await persistConversationTurn(userId, journeyId, { prompt: streamPrompt, response, timestamp: new Date().toISOString(), predictionHit });

    setStreamRequest(null);
    setStreamPrompt("");
    setBusy(false);
  }, [applyOracleResponse, cognitiveLevel, currentState, history.length, journeyId, language, pauseStartedAt, predictionHit, streamPrompt, userId]);

  const handleStreamError = useCallback(() => setBusy(false), []);

  const handlePrimaryAction = useCallback(async () => {
    const actionText = draftSelection ? `${currentResponse.primaryAction.action}: ${draftSelection}` : currentResponse.primaryAction.action;
  ...
  const handleLanguageChange = useCallback(async (nextLanguage: LanguageCode) => {
    ...
    if (profile) await persistOnboardingProfile(userId, { ...profile, preferredLanguage: nextLanguage });
  ...
  const demoAnnotation = useMemo(() => (demoMode ? demoNotes[Math.min(demoIndex, demoNotes.length - 1)] : null), [demoIndex, demoMode]);
