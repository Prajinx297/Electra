...
export const buildVoteCountFrames = ({
  ...
  return Array.from({ length: precinctCount }, (_, index) => {
    ...
    const splits = Array.from({ length: candidateCount }, (_, candidateIndex) =>
      Math.floor(baseVotes / candidateCount) + (candidateIndex === index % candidateCount ? 8 : 0)
    );
    const splitTotal = splits.reduce((sum, value) => sum + value, 0);
    const adjusted = [...splits];
    adjusted[0] += baseVotes - splitTotal;
    adjusted.forEach((value, candidateIndex) => {
...
export const calculateRecountTrigger = (
  ...
  const marginPercent = totalVotes === 0 ? 0 : Number(((marginVotes / totalVotes) * 100).toFixed(2));
  return {
    totalVotes,
    marginVotes,
    marginPercent,
